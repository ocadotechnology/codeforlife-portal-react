    ...
    if request.method == "POST":
        if "teacher_signup-teacher_email" in request.POST:
            ...
            if teacher_signup_form.is_valid():
                ...
                for field, errors in teacher_signup_form.errors.items():
...
def process_signup_form(request, data):
    ...
    is_email_found = User.objects.filter(email=email).exists()
    if email and is_email_found:
    ...
    return JsonResponse({"message": "verification needed"}, status=200)
    return render(
        request,
